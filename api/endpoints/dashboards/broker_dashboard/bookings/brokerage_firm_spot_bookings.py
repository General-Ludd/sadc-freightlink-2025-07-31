...
from services.spot_bookings.power_shipment import broker_create_spot_power_shipment
...
@router.get("/broker-access/ftl-shipment-qoute", status_code=status.HTTP_200_OK)
def broker_get_spot_ftl_shipment_qoute_endpoint(
    origin_address: str,
...
@router.post("/broker-access/spot/ftl-shipment-create", status_code=status.HTTP_201_CREATED)
def broker_access_create_spot_ftl_endpoint(
    ...
    try:
        result = broker_create_ftl_shipment(
            db,
            shipment_data,
...
@router.post("/broker-access/spot/ftl-lane-qoute", status_code=status.HTTP_200_OK)
def broker_access_get_spot_ftl_lane_qoute_endpoint(
    start_date: date,
    end_date: date,
...
@router.get("/broker-access/spot/power-shipment-qoute", status_code=status.HTTP_200_OK)
def broker_access_get_spot_power_shipment_qoute_endpoint(
    origin_address: str,
    destination_address: str,
    minimum_weight_bracket: int,
...
@router.post("/broker-access/power-shipment-create", status_code=status.HTTP_201_CREATED)
def broker_access_create_spot_power_endpoint(
    ...
    try:
        result = broker_create_spot_power_shipment(
            db,
            shipment_data,
            broker_transaction_data,
            consignor_data,
