...
@router.get("/shared-shipment-loadboard")
def get_public_shipments_loadboard(
    db: Session = Depends(get_db),
):
    try:
        loadboards = db.query(Shipment_Auction_Loadboard).filter(
...
@router.get("/shared-shipment-loadboard/{id}")
def get_public_loadboard_shipment(
    id: int,
    db: Session = Depends(get_db),
):
    try:
...
@router.get("/shared-tender-loadboard")
def get_shared_loadboard_tenders(
    db: Session = Depends(get_db),
):
    # ========================================================
    # 2. GET ALL TENDERS VISIBLE TO CARRIERS
...
@router.get("/shared-tender-loadboard/{id}")
def get_public_tender_loadboard(
    id: int,
    db: Session = Depends(get_db),
):
    try:
