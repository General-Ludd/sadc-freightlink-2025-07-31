                ...
                "exchange_lane_details": {
                    ...
                    "exchange_finance": {
                        ...
                        "bid": [{
                            ...
                            "carrier_name": f"SADC FREIGHTLINK Carrier No-{bid.carrier_id}",
...
@router.post("/exchange/ftl-lane-exchange-cancel/{exchange_id}", status_code=status.HTTP_200_OK)
def cancel_exchange_ftl_lane_exchange_endpoint(
    exchange_id: int,
    db: Session = Depends(get_db),
    current_user: dict = Depends(get_current_user),
):
...
@router.get("/client/ftl-exchange-bids/{id}")
def shipper_broker_get_exchange_bids(
    id: int,
    db: Session = Depends(get_db),
    current_user: dict = Depends(get_current_user)
):
    ...
    try:
        ...
        bids = db.query(Exchange_FTL_Shipment_Bid).filter(Exchange_FTL_Shipment_Bid.exchange_id == id).all()
        return {
            "bids": [{
                "id": bid.id,
