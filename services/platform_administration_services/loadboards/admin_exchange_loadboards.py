...
@router.get("/admin/carrier/ftl/exchange")
def admin_get_ftl_exchange_loadboard(
...
#################################################FTL Lane Loadboard###################################
#####################################################################################################
@router.get("/admin/exchange-ftl-lane-loadboard")
def admin_get_exchange_ftl_lane_loadboard(
    db: Session = Depends(get_db),
    current_admin: dict = Depends(get_current_admin)
):
    try:
