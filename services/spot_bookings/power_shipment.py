...
from models.brokerage.loadboard import Power_Load_Board
...
from schemas.brokerage.loadboard import LoadBoardEntryCreate, PowerLoadBoardEntry
...
from services.brokerage.brokerage_service import calculate_brokerage_details
...
from services.shipment_service import calculate_qoute_for_power_shipment
from services.finance.finance import handle_30_day_pay, handle_credit_card, handle_instant_eft

def create_spot_power_shipment(
        db: Session,
    ...
    trailer_check = db.query(ShipperTrailer).filter(ShipperTrailer.id == shipment_data.trailer_id).first()
    # Step 2: Check if the trailer exists
    if not trailer_check:
        raise HTTPException(
            status_code=404,
            detail=f"Trailer with ID {shipment_data.trailer_id} not found."
        )

    # Step 1 Check Trailer Information Status
    try:
        equipment_check = check_trailer_equipment_info(
            db=db,
    ...
    shipment = POWER_SHIPMENT(
        ...
        trailer_id=shipment_data.trailer_id,
        trailer_make=trailer_check.make,
        trailer_model=trailer_check.model,
        trailer_year=trailer_check.year,
        trailer_color=trailer_check.color,
        trailer_license_plate=trailer_check.license_plate,
        trailer_vin=trailer_check.license_plate,
        trailer_equipment_type=trailer_check.equipment_type,
        trailer_type=trailer_check.trailer_type,
        trailer_length=trailer_check.trailer_length,
        minimum_weight_bracket=shipment_data.minimum_weight_bracket,
    ...
    loadboard_data = PowerLoadBoardEntry(
        shipment_id=shipment.id,
        minimum_weight_bracket=shipment_data.minimum_weight_bracket,
        minimum_git_cover_amount=shipment_data.minimum_git_cover_amount,
        minimum_liability_cover_amount=shipment_data.minimum_liability_cover_amount,
    ...
    loadboard_entry = Power_Load_Board(
    ...
    return {
        ...
        "loadboard_entry": {
            ...
            "posting_price": loadboard_entry.shipment_rate,
            "rate_per_km": loadboard_entry.rate_per_km,
            "rate_per_ton": loadboard_entry.rate_per_ton,
            "payout_method": loadboard_entry.payment_terms,
            "created_at": loadboard_entry.created_at,
        },
    }


##########################################Broker Power Shipment Create##################################
def broker_create_spot_power_shipment(
        db: Session,
        shipment_data: Power_Shipment_Booking,
