...
def admin_create_client_ftl_shipment(
    ...
    try:
        if billing_account.payment_terms == "PAB":
            if billing_account.credit_balance >= quote_per_shipment:
                billing_account.credit_balance -= quote_per_shipment
            else:
                raise HTTPException(
                    status_code=402,
                    detail=f"Shipment booking failed due to insufficient funds. Please deposit at least R{quote_per_shipment:.2f} to proceed."
            ...
            projected_balance = billing_account.total_outstanding + quote_per_shipment
