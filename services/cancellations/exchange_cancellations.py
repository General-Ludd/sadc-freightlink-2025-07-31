...
from models.brokerage.loadboards.exchange_loadboards import Exchange_Ftl_Load_Board, Exchange_Power_Load_Board, Exchange_Ftl_Lane_LoadBoard
...
############################################FTL Lane Exchange Cancellation###################################
def cancel_exchange_ftl_lane_booking(exchange_id: int, db: Session, current_user: dict):
    user_id = current_user.get("id")
    company_id = current_user.get("company_id")

    # 1. Retrive Exchange
    exchange = db.query(FTL_Lane_Exchange).filter(FTL_Lane_Exchange.id == exchange_id).first()
    if not exchange:
        raise HTTPException(status_code=404, detail="Exchange not found")
    if not exchange.shipper_company_id == company_id:
        raise HTTPException(status_code=403, detail="You are not authorized to cancel this exchange")
    ...
    exchange_loadboard = db.query(Exchange_Ftl_Lane_LoadBoard).filter(Exchange_Ftl_Lane_LoadBoard.exchange_id == exchange_id).first()
    if not exchange_loadboard:
        raise HTTPException(status_code=404, detail="Exchange loadboard not found")
