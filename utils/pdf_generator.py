...
def generate_shipper_invoice_pdf(invoice: dict, logo_url: str = None) -> bytes:
    ...
    if logo_url:
        try:
            ...
            if response.status_code == 200:
                ...
                c.drawImage(logo, 30, y - 65, width=99, height=90, preserveAspectRatio=True)
    ...
    table.drawOn(c, 40, details_y - 100)
...
def generate_interim_invoice_pdf(invoice: dict, logo_url: str = None) -> bytes:
    buffer = BytesIO()
    c = canvas.Canvas(buffer, pagesize=A4)
    width, height = A4
    ...
    billed_to_text = (
        f"{invoice['billed_to']['business_name']}\n"
        f"Reg No: {invoice['billed_to'].get('registration_no', 'N/A')}\n"
        f"{invoice['billed_to']['billing_address']}\n"
        f"Email: {invoice['billed_to']['business_email']}"
    )
    ...
    details_y = section_y - 10
    c.setFont("Helvetica-Bold", 12)
    c.drawString(40, details_y, "Billing Details")

    details_data = [
        ["Contract ID", invoice["information"]["contract_id"]],
        ["Contract Type", invoice["information"]["contract_type"]],
        ["Billing Period", invoice["information"]["billing_period"]],
        ["Status", invoice["status"]],
        ["Description", invoice["description"] or "N/A"],
    ]

    table = Table(details_data, colWidths=[120, 380])
    table.setStyle(
        TableStyle(
            [
                ("GRID", (0, 0), (-1, -1), 0.25, colors.grey),
